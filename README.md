...
Browse, preview, search, and organize pages from **self-hosted AFFiNE** or **AFFiNE Cloud** without leaving VS Code or Cursor.
...
- **Favorites**, **collections** (titles), **recent pages** (last 8), and tags when the workspace has them

### Command palette

- Sign in, Search AFFiNE Pages, Open AFFiNE Document, Open AFFiNE Link, New Page, New Folder, Restore from trash, Sync, Force Reload, Sign out
- Move, trash, rename, and duplicate stay on the tree / sidebar (not in the palette)
...
- Title, tags, creator, and last editor (deduped when they are the same person), with avatars
- Headings 1–6, paragraphs, quotes, bullets, numbered lists, and to-do checkboxes
- Bold, italic, underline, strike, inline code, highlight, text color, and links
- Code blocks, dividers, bookmarks, LaTeX source, tables, callouts
- Same-origin images; attachments listed by filename and size (open in the browser)
...
Databases, column layouts, embeds, and whiteboard drawing are not drawn in preview — open the page in AFFiNE. See [FEATURES.md](./FEATURES.md#local-preview).

### Organize

- **New page** and **new Organize folder** (sidebar, Pages toolbar, or command palette)
- **Rename** and **duplicate** a page (Pages context menu)
- Move a page to another Organize folder (sidebar hover or Pages tree)
- Send a page to **AFFiNE trash** (not a permanent delete); confirm, then a spinner on that row
- **Restore** a page from trash (sidebar or command palette)
- Writes go over Socket.IO / Yjs, same as the AFFiNE web app (`affine.clientVersion` ≥ 0.26.0)
- Open the real AFFiNE editor in your **system browser** (login session stays there)
- Hover an AFFiNE page URL in the editor to see the catalog title; **Open AFFiNE Link** from the context menu
...
The same contribution kit is used by the [Plane VS Code client](https://github.com/masteryhub-its/plane-vscode-extension).
...
- An AFFiNE account on Cloud or a self-hosted server
